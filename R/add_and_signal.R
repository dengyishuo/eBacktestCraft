#' Add AND-Combined Signal
#'
#' Merges multiple existing 0/1 signal columns with AND logic: the output signal
#' is 1 only when **all** input signals are 1 simultaneously. Use this to stack
#' independent filters (e.g., momentum filter AND regime filter AND TD signal).
#'
...
#' df <- add_regime_signal(df, close_col = "adjusted", n = 60)
#' result <- add_and_signal(df,
#'   signal_cols = c("signal_mom_20_gt_0", "signal_regime_ma60"))
#'
#' @family signal-composite
#' @importFrom tibble as_tibble
#' @export
add_and_signal <- function(mkt_data,
                           signal_cols,
                           signal_name = NULL,
  ...
  col <- if (is.null(signal_name))
    paste0("signal_and_", paste(signal_cols, collapse = "_"))
  else signal_name
